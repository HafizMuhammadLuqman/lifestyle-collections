...
// ============================================================
// STORE CONFIG — Admin panel se edit hoga
// ============================================================
const DEFAULT_STORE = {
...
function Toast({ msg }) {
  ...
  return (
    <div style={{
      position:"fixed",top:20,right:20,background:"#222",color:"#fff",
      padding:"12px 20px",borderRadius:10,zIndex:99999,fontSize:14,
      boxShadow:"0 4px 24px rgba(0,0,0,.35)",animation:"toastIn .3s ease"
    }}>{msg}</div>
  );
}
...
function useFirebaseData() {
  ...
  useEffect(() => {
    ...
    const unsubCfg = onValue(cfgRef, (snap) => {
      if (snap.exists()) {
        setStoreConfig({ ...DEFAULT_STORE, ...snap.val() });
      }
    }, () => {});

    return () => { unsubProd(); unsubOrd(); unsubCfg(); };
  }, []);

  const saveProduct = async (product, fbKey = null) => {
    if (fbKey) {
      await update(ref(db, `products/${fbKey}`), product);
    } else {
      const key = `prod_${Date.now()}`;
      await set(ref(db, `products/${key}`), { ...product, id: Date.now() });
    }
  };

  const deleteProduct = async (fbKey) => {
    await set(ref(db, `products/${fbKey}`), null);
  };

  const saveOrder = async (order) => {
    const key = `order_${Date.now()}`;
    await set(ref(db, `orders/${key}`), { ...order, timestamp: Date.now() });
    return order;
  };

  const updateOrderStatus = async (fbKey, status) => {
    await update(ref(db, `orders/${fbKey}`), { status });
  };

  const saveStoreConfig = async (config) => {
    await set(ref(db, "storeConfig"), config);
  };

  return { products, orders, storeConfig, loading, saveProduct, deleteProduct, saveOrder, updateOrderStatus, saveStoreConfig };
}

export default function App() {
  const { products, orders, storeConfig, loading, saveProduct, deleteProduct, saveOrder, updateOrderStatus, saveStoreConfig } = useFirebaseData();
  ...
  if (loading) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:"#fff8f5"}}>
      <div style={{textAlign:"center"}}>
        ...
        <div style={{fontSize:13,color:"#999",marginTop:8}}>Firebase se data load ho raha hai...</div>
  ...
  if (adminIn && page.startsWith("admin")) return (
    <AdminPanel
      page={page} go={go} products={products} orders={orders}
      storeConfig={STORE}
      saveProduct={saveProduct} deleteProduct={deleteProduct}
      updateOrderStatus={updateOrderStatus} saveStoreConfig={saveStoreConfig}
      editingProduct={editingProduct} showToast={showToast}
      onLogout={()=>{setAdminIn(false);go("home");}}
    />
  );

  if (page==="checkout") return (
    <CheckoutPage cart={cart} cartTotal={cartTotal} STORE={STORE} onBack={()=>go("home")}
      updateQty={updateQty} removeCart={removeCart}
  ...
  if (page==="detail" && selProduct) return (
    <ProductDetail product={selProduct} STORE={STORE} onBack={()=>go("home")}
      onAddCart={addCart} isWishlisted={wishlist.includes(selProduct.id)}
    ...
    <div style={{fontFamily:"'Segoe UI',Tahoma,sans-serif",background:"#F5F5F5",minHeight:"100vh"}}>
      ...
      <header style={{background:"#FF6B35",position:"sticky",top:0,zIndex:200,boxShadow:"0 2px 12px rgba(0,0,0,.2)"}}>
        <div style={{background:"#E85520",padding:"4px 16px",display:"flex",justifyContent:"space-between",fontSize:12,color:"rgba(255,255,255,.9)",flexWrap:"wrap",gap:4}}>
          <span>🚚 Delivery charges: Rs. {STORE.delivery} all over Pakistan</span>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 16px",maxWidth:1300,margin:"0 auto",width:"100%",boxSizing:"border-box"}}>
          ...
          <div style={{display:"flex",gap:6,flexShrink:0}}>
            ...
            <button onClick={()=>go("admin")}
              style={{background:"rgba(255,255,255,.2)",border:"none",borderRadius:8,padding:"7px 10px",color:"#fff",cursor:"pointer",fontSize:12,fontWeight:600}}>
              🔒 Admin
            </button>
            <button onClick={()=>go("checkout")}
              style={{background:"#fff",border:"none",borderRadius:8,padding:"7px 10px",color:"#FF6B35",cursor:"pointer",fontSize:12,fontWeight:700,display:"flex",alignItems:"center",gap:4}}>
              🛒 {cartCount>0 && <span style={{background:"#FF6B35",color:"#fff",borderRadius:"50%",width:18,height:18,fontSize:10,display:"flex",alignItems:"center",justifyContent:"center"}}>{cartCount}</span>}
              Cart
            </button>
          </div>
        </div>
      ...
      <div style={{maxWidth:1300,margin:"0 auto",padding:"0 10px"}}>
          ...
          <div style={{position:"absolute",bottom:8,left:"50%",transform:"translateX(-50%)",display:"flex",gap:6}}>
            {banners.map((_, i) => (
              <div key={i} onClick={()=>setBannerIdx(i)}
                style={{width:i===bannerIdx%banners.length?20:7,height:7,borderRadius:4,background:i===bannerIdx%banners.length?"#fff":"rgba(255,255,255,.5)",cursor:"pointer",transition:"all .3s"}} />
            ))}
          </div>
        </div>

        <div style={{background:"#fff",borderRadius:14,padding:16,marginBottom:12}}>
          <h3 style={{margin:"0 0 12px",fontSize:16,color:"#222"}}>🗂️ Categories</h3>
          <div style={{display:"flex",overflowX:"auto",gap:8,paddingBottom:4,WebkitOverflowScrolling:"touch",scrollbarWidth:"none"}}>
            {CATEGORIES.map(cat=>(
              <div key={cat.name} onClick={()=>setActiveCategory(cat.name)}
                style={{textAlign:"center",cursor:"pointer",padding:"10px 12px",borderRadius:12,background:activeCategory===cat.name?"#FFF5F0":"#fafafa",border:activeCategory===cat.name?`2px solid ${cat.color}`:"2px solid transparent",transition:"all .2s",flexShrink:0,minWidth:70}}>
                <div style={{fontSize:22}}>{cat.icon}</div>
                <div style={{fontSize:10,marginTop:4,fontWeight:500,color:"#555",lineHeight:1.2,whiteSpace:"nowrap"}}>{cat.name.split(" ")[0]}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{background:"#fff",borderRadius:14,padding:16,marginBottom:12}}>
          <h3 style={{margin:0,fontSize:17,fontWeight:800,marginBottom:14}}>
            {activeCategory==="All" ? "⚡ All Products" : `${CATEGORIES.find(c=>c.name===activeCategory)?.icon||""} ${activeCategory}`}
          ...
          {filteredProds.length===0 ? (
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:12}}>
              {filteredProds.map(p=>(
                <ProductCard key={p.fbKey||p.id} product={p}
                  onAdd={addCart} onDetail={()=>go("detail",{product:p})}
        ...
        <footer style={{background:"#1A1A1A",borderRadius:14,padding:24,color:"#bbb",marginBottom:12}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:16,marginBottom:16}}>
            ...
            <div>
              <div style={{color:"#fff",fontWeight:600,marginBottom:8,fontSize:13}}>Quick Links</div>
              {["Home","All Products","Fashion","Electronics","Home & Kitchen"].map(i=>(
                <div key={i} style={{fontSize:12,marginBottom:5,cursor:"pointer",color:"#aaa"}}>{i}</div>
              ))}
            </div>
            <div>
...
function ProductCard({ product:p, onAdd, onDetail, isWishlisted, onWishlist }) {
  ...
  return (
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{background:"#fff",borderRadius:14,overflow:"hidden",cursor:"pointer",border:"1px solid #f0f0f0",
        boxShadow:hov?"0 8px 28px rgba(0,0,0,.12)":"0 2px 6px rgba(0,0,0,.05)",
        transform:hov?"translateY(-3px)":"none",transition:"all .25s",position:"relative",display:"flex",flexDirection:"column"}}>
      ...
      <button onClick={e=>{e.stopPropagation();onWishlist(p.id);}}
        style={{position:"absolute",top:6,right:6,background:"rgba(255,255,255,.9)",border:"none",borderRadius:"50%",width:28,height:28,cursor:"pointer",fontSize:14,zIndex:2}}>
        {isWishlisted?"❤️":"🤍"}
      </button>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:6,padding:"6px 10px",background:"#fafafa",borderTop:"1px solid #f0f0f0"}} onClick={e=>e.stopPropagation()}>
        <button onClick={e=>{e.stopPropagation();setQty(q=>Math.max(1,q-1));}}
          style={{width:26,height:26,borderRadius:"50%",border:"1px solid #ddd",background:"#fff",cursor:"pointer",fontSize:14,fontWeight:700,color:"#FF6B35",display:"flex",alignItems:"center",justifyContent:"center"}}>−</button>
      ...
      <button onClick={handleAdd}
        style={{width:"100%",background:added?"#4CAF50":"#FF6B35",border:"none",color:"#fff",padding:"9px",fontSize:12,fontWeight:600,cursor:"pointer",transition:"background .3s"}}>
        ...
        <div style={{background:"#fff",borderRadius:14,padding:20,display:"flex",gap:24,flexWrap:"wrap"}}>
          ...
          <div style={{flex:1,minWidth:220}}>
            ...
            <div style={{display:"flex",gap:10,marginBottom:16}}>
              <button onClick={()=>onAddCart(p,qty)} style={{flex:1,background:"#FF6B35",border:"none",color:"#fff",padding:"12px 16px",borderRadius:10,fontSize:14,fontWeight:700,cursor:"pointer"}}>
                🛒 Add to Cart ({qty})
              </button>
            ...
            <div style={{background:"#FFF5F0",borderRadius:10,padding:12,fontSize:12,color:"#555",lineHeight:2}}>
              🚚 Delivery: Rs. {STORE.delivery} all over Pakistan<br/>
              🔄 7-day easy returns<br/>
              ✅ Authentic product guarantee<br/>
...
function AdminPanel({ page, go, products, orders, storeConfig, saveProduct, deleteProduct, updateOrderStatus, saveStoreConfig, editingProduct, showToast, onLogout }) {
  ...
  return (
    <div style={{fontFamily:"'Segoe UI',sans-serif",display:"flex",minHeight:"100vh",background:"#F0F2F5"}}>
      ...
      <div style={{flex:1,padding:20,overflowY:"auto"}}>
        ...
        {page==="adminAddProd" && (
          <AddProductForm editingProduct={editingProduct} showToast={showToast}
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,fontSize:12}}>
                    <div style={{background:"#F5F5F5",borderRadius:8,padding:10}}>
                      ...
                      <div style={{color:"#555",lineHeight:1.9}}>
                        ...
                        <b>Phone:</b> {order.customer?.phone}<br/>
                        <b>Address:</b> {order.customer?.house}, {order.customer?.area}, {order.customer?.city}
                      </div>
                    </div>
                    <div style={{background:"#F5F5F5",borderRadius:8,padding:10}}>
                      <div style={{fontWeight:600,marginBottom:5,color:"#555"}}>💳 Payment</div>
                      <div style={{color:"#555",lineHeight:1.9}}>
                        <b>Method:</b> {order.customer?.payMethod==="COD"?"Cash on Delivery":order.customer?.payMethod==="jazz"?"JazzCash":"EasyPaisa"}<br/>
                        {order.customer?.transId && <><b>Trans ID:</b> {order.customer.transId}<br/></>}
                        <b>Total:</b> <span style={{color:"#FF6B35",fontWeight:700}}>{formatPrice(order.total)}</span>
                      </div>
                    </div>
                  </div>
...
function AddProductForm({ editingProduct, showToast, onSave, onDone }) {
